...
class terminalesIEWPyme
{
    public function cargarTerminalesIEWPyme($version): bool
    {
        $retorno = false;
        ...
        $plantilla = json_decode(file_get_contents(getcwd() . "/plantillas/terminales_iew_pyme.json"));
        ...
        try {
            ...
            for ($i = $filaInicio; $i < $nFilas; $i++) {
                ...
                if (strlen($marca) > 0) {
                    $c->next_result();
                    $r = $c->query("call or_terminales_iew_guardar({$version},'{$marca}','{$modelo}','{$gama}','{$precioCesion}',
