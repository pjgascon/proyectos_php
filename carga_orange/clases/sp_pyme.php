...
class terminalesSolucionPersonalizadayme
{
    public function cargarTerminaleSpPyme($version): bool
    {
        $retorno = false;

        echo "Comienza la carga\n";
        echo "Catálogo de terminales Solucion Personalizada Pyme\n";

        $c = new Conexion();
        $c->conectar();

        $plantilla = json_decode(file_get_contents(getcwd() . "/plantillas/terminales_sp_pyme.json"));
        ...
        try {
            ...
            for ($i = $filaInicio; $i < $nFilas; $i++) {
                ...
                if (strlen($marca) > 0) {
                    $c->next_result();
                    $r = $c->query("call or_terminales_solucion_personalizada_guardar({$version},'{$marca}','{$modelo}','{$gama}','{$precioCesion}',
