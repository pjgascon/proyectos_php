...
try {
    ...
    $spreadsheet = IOFactory::load(getcwd() . '/archivos_carga/subvencion_optima_36.xlsx');
    ...
    for ($i = $filaInicio; $i <= $nFilas; $i++) {
        ...
        $captTpValue = $sheet->getCell($capta_tp . $i)->getValue();
        ...
        $portaTpValue = $sheet->getCell($porta_tp . $i)->getValue();
        ...
        if (!is_numeric(strval($captTpValue))) $captTpValue = -1;
        ...
        if (!is_numeric(strval($portaTpValue))) $portaTpValue = -1;
        ...
        if (strlen($marcaValue) > 0) {
            $c->next_result();
            $r = $c->query("call orange.or_terminales_optima_36_guardar({$version},'{$marcaValue}','{$modeloValue}','{$gamaValue}','{$precioCesionValue}',
                        '{$captEpValue}','{$captNpValue}','{$captVpValue}','{$captPpValue}','{$captTpValue}',
                        '{$portaEpValue}','{$portaNpValue}','{$portaVpValue}','{$portaPpValue}','{$portaTpValue}');");
        } else {
            break;
        }
    }
