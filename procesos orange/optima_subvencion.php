...
try {
    ...
    $spreadsheet = IOFactory::load(getcwd() . '/archivos_carga/subvencion_optima.xlsx');
    ...
    for ($i = $filaInicio; $i <= $nFilas; $i++) {
        ...
        if (strlen($marcaValue) > 0) {
            $c->next_result();
            $r = $c->query("call orange.or_terminales_optima_guardar({$version},'{$marcaValue}','{$modeloValue}','{$gamaValue}','{$precioCesionValue}',
